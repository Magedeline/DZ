...
public static class ChapterProgressionManager
{
    ...
    private static IEnumerator OnPerformCh9Unlock(On.Celeste.OuiChapterSelect.orig_PerformCh9Unlock orig, OuiChapterSelect self, bool easeCamera = true)
    {
        ...
        if (!global::Celeste.MaggyHelperSaveFacade.IsChapterUnlocked(Ch10Sid))
        {
    ...
    private static void ApplyProgressionUnlocks(global::Celeste.Mod.MaggyHelper.MaggyHelperModuleSaveData save)
    {
        if (save.UnlockedChapter10 && !global::Celeste.MaggyHelperSaveFacade.IsChapterUnlocked(Ch10Sid))
        {
            UnlockChapter(Ch10Sid);
            save.PendingUnlockChapter10OnRestart = false;
        ...
        if (save.BossRushUnlocked && !global::Celeste.MaggyHelperSaveFacade.IsChapterUnlocked(Ch19Sid))
        {
            UnlockChapter(Ch19Sid);
            save.UnlockedChapter19 = true;
            save.PendingUnlockChapter19OnRestart = false;
        ...
        if (save.FinalDlcContentUnlocked && !global::Celeste.MaggyHelperSaveFacade.IsChapterUnlocked(Ch20Sid))
        {
            UnlockChapter(Ch20Sid);
            save.VoidMoonUnlocked = true;
            save.PendingUnlockChapter20OnRestart = false;
        ...
        if (save.TrueFinaleUnlocked && !global::Celeste.MaggyHelperSaveFacade.IsChapterUnlocked(Ch21Sid))
        {
            UnlockChapter(Ch21Sid);
            save.UnlockedChapter21 = true;
            save.PendingUnlockChapter21OnRestart = false;
    ...
    {
        global::Celeste.MaggyHelperSaveFacade.UnlockChapter(sid);
    }
    ...
    {
        if (_forcingSelection || !global::Celeste.MaggyHelperSaveFacade.IsLoaded)
            return;

        int selectedArea = global::Celeste.MaggyHelperSaveFacade.SelectedAreaId;
        ...
        try
        {
            global::Celeste.MaggyHelperSaveFacade.TrySelectArea(fallbackArea);
        }
        finally
        {
    ...
    {
        if (!global::Celeste.MaggyHelperSaveFacade.HasModSave)
            return false;

        var save = global::Celeste.Mod.MaggyHelper.MaggyHelperModule.SaveData;

        if (sid.Equals(Ch10Sid, StringComparison.OrdinalIgnoreCase))
            return !global::Celeste.MaggyHelperSaveFacade.IsChapterUnlocked(Ch10Sid)
                && save?.UnlockedChapter10 != true;

        if (sid.Equals(Ch16Sid, StringComparison.OrdinalIgnoreCase))
            return !global::Celeste.MaggyHelperSaveFacade.IsChapterUnlocked(Ch16Sid);

        if (sid.Equals(Ch19Sid, StringComparison.OrdinalIgnoreCase))
            return !global::Celeste.MaggyHelperSaveFacade.IsChapterUnlocked(Ch19Sid)
                && save?.BossRushUnlocked != true;

        if (sid.Equals(Ch20Sid, StringComparison.OrdinalIgnoreCase))
            return !global::Celeste.MaggyHelperSaveFacade.IsChapterUnlocked(Ch20Sid)
                && save?.FinalDlcContentUnlocked != true;

        if (sid.Equals(Ch21Sid, StringComparison.OrdinalIgnoreCase))
            return !global::Celeste.MaggyHelperSaveFacade.IsChapterUnlocked(Ch21Sid)
                && save?.TrueFinaleUnlocked != true;
    ...
    private static int FindNearestUnlockedArea(int fromArea)
    {
        ...
        if (origin?.SID == null)
            return global::Celeste.MaggyHelperSaveFacade.SelectedAreaId;
    ...
    private static void CmdChapterTest(string action = "status")
    {
        ...
        bool unlocked16 = global::Celeste.MaggyHelperSaveFacade.IsChapterUnlocked(Ch16Sid);
        bool unlocked19 = global::Celeste.MaggyHelperSaveFacade.IsChapterUnlocked(Ch19Sid);
        bool unlocked20 = global::Celeste.MaggyHelperSaveFacade.IsChapterUnlocked(Ch20Sid);
        bool unlocked21 = global::Celeste.MaggyHelperSaveFacade.IsChapterUnlocked(Ch21Sid);
    ...
    private static void CmdUnlockDSide(string mode = "dside")
    {
        ...
        var maggySave = global::Celeste.Mod.MaggyHelper.MaggyHelperModule.SaveData;
        ...
        if (mode == "status")
        {
            ...
            Engine.Commands?.Log($"[MaggyHelper] Checked {count} KIRBY_CELESTE chapter(s).");
            return;
        }
    ...
    private static void CmdMountainWarp()
    {
        ...
        if (targetArea == null)
        {
            Engine.Commands?.Log("[MaggyHelper] No KIRBY_CELESTE chapters found!");
            return;
        }
    ...
    private static void CmdUnlockDZ()
    {
        ...
        foreach (var area in AreaData.Areas)
        {
            if (area?.SID != null && AreaModeExtender.IsOurMap(area))
            {
                if (!global::Celeste.MaggyHelperSaveFacade.IsChapterUnlocked(area.SID))
                {
