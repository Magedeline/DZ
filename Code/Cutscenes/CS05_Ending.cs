...
public class CS05_Ending : CutsceneEntity
{
    ...
    private IEnumerator Cutscene(Level level)
    {
        ...
        yield return 0.5f;
        yield return Textbox.Say("MAGGYHELPER_CH5_OSHIRO_CHASE_END", GhostSmash);
        ...
        yield return 1.5f;
        Audio.SetMusic("guid://{45527493-aac5-4396-bb05-6ecc087b3882}");
        yield return 3f;
        ...
        yield return 0.6f;
        yield return Textbox.Say("MAGGYHELPER_CH5_ENDING", OshiroTurns);
        Add(new Coroutine(CutsceneEntity.CameraTo(level.Camera.Position + new Vector2(-80f, 0f), 3f)));
        yield return 0.5f;
        oshiroSprite.Scale.X = -1f;
        yield return 0.2f;
        t = 0f;
        oshiro.Add(new SoundSource("guid://{dfc82939-d51a-4454-aedf-1b3c6d4cf986}"));
    ...
    private IEnumerator GhostSmash(float topDelay, bool final)
    {
        ...
        if (final)
        {
            smashSfx = Audio.Play("guid://{53cd822c-b7e1-4d50-8574-4fe404457747}", angyOshiro.Position);
        }
        else
        {
            smashSfx = Audio.Play("guid://{83376c6a-a014-418e-9e21-dbc48df90069}", angyOshiro.Position);
        }
