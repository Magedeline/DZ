-- Legacy alias: MaggyHelper/ElsTrueFinalBoss → MaggyHelper/SiamoZeroFinalBoss
-- Keeps old maps displaying correctly in Lönn.
-- All new placements should use siamo_zero_final_boss.lua instead.

local utils = require("utils")
local siamoZero = require("mods").requireFromPlugin("entities.siamo_zero_final_boss")

local legacy = {}

for k, v in pairs(siamoZero) do
    legacy[k] = v
end

legacy.name = "MaggyHelper/ElsTrueFinalBoss"

-- Hide from the placement menu so mappers only see the new name
