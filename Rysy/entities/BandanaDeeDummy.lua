-- RySy plugin for MaggyHelper - BandanaDeeDummy
local Entity = {}

Entity.name = "MaggyHelper/BandanaDeeDummy"
Entity.depth = 0

function Entity.place(room, entity)
    return {
        x = entity.x or 0,
        y = entity.y or 0,
        facing = entity.facing or 1,
        animation = entity.animation or "idle",
        scale = entity.scale or 1.0,
        alpha = entity.alpha or 1.0,
        isVisible = entity.isVisible ~= false,
        playAnimationOnSpawn = entity.playAnimationOnSpawn ~= false,
        autoFollow = entity.autoFollow ~= false,
        followDelay = entity.followDelay or 0.42
    }
end

function Entity.draw(room, entity)
    local x, y = entity.x or 0, entity.y or 0
    local facing = entity.facing or 1
    local scale = entity.scale or 1.0
    return {
        texture = "characters/kirby/bandana_dee/idle00",
        x = x,
        y = y,
        justificationX = 0.5,
        justificationY = 1.0,
        scaleX = facing * scale,
        scaleY = scale
    }
end

return Entity
