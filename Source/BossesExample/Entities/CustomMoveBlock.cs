...
// Type: global::Celeste.Mod.ricky06ModPack.Entities.CustomMoveBlock
...
#nullable disable
namespace Celeste.Mod.DZ.BossesExample.Entities;

[CustomEntity(new string[] {"ricky06/CustomMoveBlock"})]
[Tracked(false)]
public class CustomMoveBlock : Solid
{
  ...
  private bool fast;
  private CustomMoveBlock.Directions direction;
  private float homeAngle;
  private int angleSteerSign;
  private Vector2 startPosition;
  private CustomMoveBlock.MovementState state;
  ...
  private CustomMoveBlock.Border border;
  private Color fillColor = CustomMoveBlock.idleBgFill;
  private float flash;
  ...
  private bool lessParticles;

  public CustomMoveBlock(
    Vector2 position,
    int width,
    int height,
    CustomMoveBlock.Directions direction,
    bool canSteer,
    bool fast,
    float speed,
    float respawnSpeed,
    ...
    : base(position, (float) width, (float) height, false)
  {
    ...
    this.lessParticles = lessParticles;
    switch (direction)
    {
      case CustomMoveBlock.Directions.Left:
        this.homeAngle = this.targetAngle = this.angle = 3.14159274f;
        this.angleSteerSign = -1;
        break;
      case CustomMoveBlock.Directions.Up:
        this.homeAngle = this.targetAngle = this.angle = -1.57079637f;
        this.angleSteerSign = 1;
        break;
      case CustomMoveBlock.Directions.Down:
        this.homeAngle = this.targetAngle = this.angle = 1.57079637f;
        this.angleSteerSign = -1;
        break;
      default:
    ...
    int num1 = width / 8;
    int num2 = height / 8;
    MTexture mtexture1 = GFX.Game["objects/DZ/DZ/moveBlock/base"];
    MTexture mtexture2 = GFX.Game["objects/DZ/DZ/moveBlock/button"];
    if (canSteer && (direction == CustomMoveBlock.Directions.Left || direction == CustomMoveBlock.Directions.Right))
    {
      ...
      mtexture1 = GFX.Game["objects/DZ/DZ/moveBlock/base_h"];
    }
    else if (canSteer && (direction == CustomMoveBlock.Directions.Up || direction == CustomMoveBlock.Directions.Down))
    {
      ...
      mtexture1 = GFX.Game["objects/DZ/DZ/moveBlock/base_v"];
    }
    ...
    this.arrows = GFX.Game.GetAtlasSubtextures("objects/DZ/DZ/moveBlock/arrow");
  ...
  public CustomMoveBlock(EntityData data, Vector2 offset)
    : this(((data.Position) + (offset)), data.Width, data.Height, data.Enum<CustomMoveBlock.Directions>(nameof (direction), CustomMoveBlock.Directions.Left), data.Bool(nameof (canSteer), true), data.Bool(nameof (fast), false), data.Float(nameof (moveSpeed), 0.0f), data.Float(nameof (respawnSpeed), 0.0f), data.Bool(nameof (lessParticles), false))
  {
  }
    ...
    while (true)
    {
      ...
      while (true)
      {
        if (this.canSteer)
        {
          ...
          bool flag = this.direction == CustomMoveBlock.Directions.Right || this.direction == CustomMoveBlock.Directions.Left ? this.HasPlayerOnTop() : this.HasPlayerClimbing();
          if (flag && (double) noSteerTimer > 0.0)
            noSteerTimer -= Engine.DeltaTime;
          if (flag)
          {
            if ((double) noSteerTimer <= 0.0)
              this.targetAngle = this.direction != CustomMoveBlock.Directions.Right && this.direction != CustomMoveBlock.Directions.Left ? this.homeAngle + 0.7853982f * (float) this.angleSteerSign * (float) Input.MoveX.Value : this.homeAngle + 0.7853982f * (float) this.angleSteerSign * (float) Input.MoveY.Value;
          }
          else
            noSteerTimer = 0.2f;
        }
        ...
        bool flag2;
        Rectangle bounds;
        if (this.direction == CustomMoveBlock.Directions.Right || this.direction == CustomMoveBlock.Directions.Left)
        {
        ...
        else
        {
          ...
          int num1;
          if (this.direction == CustomMoveBlock.Directions.Down)
          {
...
label_43:
      ...
      yield return (object) 0.2f;
      this.BreakParticles();
      List<CustomMoveBlock.Debris> debris = new List<CustomMoveBlock.Debris>();
      int amount = this.lessParticles ? 100 : 8;
      for (int i = 0; (double) i < (double) ((Entity) this).Width; i += amount)
      {
        for (int j = 0; (double) j < (double) ((Entity) this).Height; j += amount)
        {
          Vector2 vector2 = new Vector2((float) i + 4f, (float) j + 4f);
          CustomMoveBlock.Debris debris2 = Engine.Pooler.Create<CustomMoveBlock.Debris>().Init(((((Entity) this).Position) + (vector2)), ((Entity) this).Center, ((this.startPosition) + (vector2)));
          debris.Add(debris2);
          ((Entity) this).Scene.Add((Entity) debris2);
          vector2 = new Vector2();
          debris2 = (CustomMoveBlock.Debris) null;
        }
      }
      ...
      yield return (object) this.respawnSpeed;
      foreach (CustomMoveBlock.Debris item in debris)
        item.StopMoving();
      ...
      foreach (CustomMoveBlock.Debris item2 in debris)
        item2.StartShaking();
      yield return (object) 0.2f;
      foreach (CustomMoveBlock.Debris item3 in debris)
      ...
      this.angle = this.targetAngle = this.homeAngle;
      this.noSquish = (Player) null;
      this.fillColor = CustomMoveBlock.idleBgFill;
      this.UpdateColors();
      this.flash = 1f;
      debris = (List<CustomMoveBlock.Debris>) null;
      instance = (EventInstance) null;
      moveBlock = (CustomMoveBlock) null;
  ...
  private IEnumerator SoundFollowsDebrisCenter(
    ...
    List<CustomMoveBlock.Debris> debris)
  {
    while (true)
    {
  ...
  public override void Update()
  {
    ...
    if (this.canSteer)
    {
      bool flag1 = (this.direction == CustomMoveBlock.Directions.Up || this.direction == CustomMoveBlock.Directions.Down) && ((Entity) this).CollideCheck<Player>(((((Entity) this).Position) + (new Vector2(-1f, 0.0f))));
      bool flag2 = (this.direction == CustomMoveBlock.Directions.Up || this.direction == CustomMoveBlock.Directions.Down) && ((Entity) this).CollideCheck<Player>(((((Entity) this).Position) + (new Vector2(1f, 0.0f))));
      bool flag3 = (this.direction == CustomMoveBlock.Directions.Left || this.direction == CustomMoveBlock.Directions.Right) && ((Entity) this).CollideCheck<Player>(((((Entity) this).Position) + (new Vector2(0.0f, -1f))));
  ...
  {
    Color color = CustomMoveBlock.idleBgFill;
    if (this.state == CustomMoveBlock.MovementState.Moving)
      color = CustomMoveBlock.pressedBgFill;
    else if (this.state == CustomMoveBlock.MovementState.Breaking)
      color = CustomMoveBlock.breakingBgFill;
  ...
  public override void Render()
  {
    ...
    if (this.state != CustomMoveBlock.MovementState.Breaking)
      ...
      GFX.Game["objects/DZ/DZ/moveBlock/x"].DrawCentered(((Entity) this).Center);
  ...
  {
    bool flag1 = this.direction == CustomMoveBlock.Directions.Down || this.direction == CustomMoveBlock.Directions.Up;
    ...
    if (flag2)
      ((Entity) this).SceneAs<Level>().ParticlesBG.Emit(CustomMoveBlock.P_Activate, (int) ((double) ((Entity) this).Height / 2.0), ((Entity) this).CenterLeft, ((((Vector2.UnitY) * (((Entity) this).Height - 4f))) * (0.5f)), 3.14159274f);
    if (flag3)
      ((Entity) this).SceneAs<Level>().ParticlesBG.Emit(CustomMoveBlock.P_Activate, (int) ((double) ((Entity) this).Height / 2.0), ((Entity) this).CenterRight, ((((Vector2.UnitY) * (((Entity) this).Height - 4f))) * (0.5f)), 0.0f);
    if (flag4)
      ((Entity) this).SceneAs<Level>().ParticlesBG.Emit(CustomMoveBlock.P_Activate, (int) ((double) ((Entity) this).Width / 2.0), ((Entity) this).TopCenter, ((((Vector2.UnitX) * (((Entity) this).Width - 4f))) * (0.5f)), -1.57079637f);
    ((Entity) this).SceneAs<Level>().ParticlesBG.Emit(CustomMoveBlock.P_Activate, (int) ((double) ((Entity) this).Width / 2.0), ((Entity) this).BottomCenter, ((((Vector2.UnitX) * (((Entity) this).Width - 4f))) * (0.5f)), 1.57079637f);
  }
  ...
  private void BreakParticles()
  {
    ...
    for (int index1 = 0; (double) index1 < (double) ((Entity) this).Width; index1 += 4)
    {
      for (int index2 = 0; (double) index2 < (double) ((Entity) this).Height; index2 += 4)
      {
        ...
        ((Entity) this).SceneAs<Level>().Particles.Emit(CustomMoveBlock.P_Break, 1, vector2, ((Vector2.One) * (2f)), Calc.Angle(((vector2) - (center))));
      }
    }
  }
  ...
  private void MoveParticles()
  {
    ...
    float num1;
    float num2;
    if (this.direction == CustomMoveBlock.Directions.Right)
    {
    ...
    else if (this.direction == CustomMoveBlock.Directions.Left)
    {
    ...
    else if (this.direction == CustomMoveBlock.Directions.Down)
    {
    ...
    if (particleRemainder <= 0)
      return;
    ((Entity) this).SceneAs<Level>().ParticlesBG.Emit(CustomMoveBlock.P_Move, particleRemainder, vector2_1, vector2_3, num1);
  }
  ...
  private class Debris : Actor
  {
      ...
      : base(Vector2.Zero)
    {
      ...
      ((Entity) this).Add((Component) (this.sprite = new Image(Calc.Choose<MTexture>(Calc.Random, GFX.Game.GetAtlasSubtextures("objects/DZ/DZ/moveblock/debris")))));
    ...
    public CustomMoveBlock.Debris Init(Vector2 position, Vector2 center, Vector2 returnTo)
    {
      ((Entity) this).Collidable = true;
      ((Entity) this).Position = position;
