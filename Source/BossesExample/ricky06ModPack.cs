...
using global::Celeste.Mod.DZ.BossesExample.Entities;
using Microsoft.Xna.Framework;
using Monocle;
using MonoMod.RuntimeDetour;

namespace Celeste.Mod.DZ.BossesExample;
...
public static class BossesExampleModule
{
    ...
    private static void TryHookPlayerMethod(string methodName, string detourName, ref Hook hook)
    {
        ...
        {
            Logger.Log(LogLevel.Warn, "DZ", $"[BossesExample] Failed to hook Player.{methodName}: {ex.Message}");
        }
    }
    ...
    private static SpriteBank TryLoadBank(Atlas atlas, string path)
    {
        ...
        {
            Logger.Log(LogLevel.Warn, "DZ", $"[BossesExample] Missing sprite bank '{path}', using placeholders. {ex.Message}");
            return null;
        }
    }
    ...
    {
        public bool ResetKeysForSession => DZModule.Settings?.BossesExampleResetKeysForSession == true;
    }

    public sealed class BossesExampleSaveDataProxy
    {
        public HashSet<string> StoneFlags => DZModule.SaveData?.BossesExampleStoneFlags ?? FallbackStoneFlags;
    }
