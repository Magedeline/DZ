...
namespace Celeste.Cutscenes
{
	...
	public class CS20_Ending : CutsceneEntity
	{
		...
		public CS20_Ending(Player player) : base(false, true)
		{
			base.Tag = Tags.HUD;
		...
		private IEnumerator Cutscene(Level level)
		{
			...
			yield return 1f;
			this.Atlas = Atlas.FromAtlas(Path.Combine("Graphics", "Atlases", "TheEnd"), Atlas.AtlasDataFormat.PackerNoAtlas);
			...
			yield return 2f;
			this.cinIntro = Audio.Play("event:/desolozantas/final_content/music/lvl20/cinematic/end_intro");
			Audio.SetAmbience(null, true);
			this.counting = true;
			...
			yield return 1f;
			yield return Textbox.Say("CH20_END_CINEMATIC", new Func<IEnumerator>[]
			{
		...
		private IEnumerator EndingRoutine()
		{
			...
			// Transition to the white cymbal fade video cutscene
			Player player = this.Level.Tracker.GetEntity<Player>();
			
            Level.TeleportTo(player, "end-video", Player.IntroTypes.Transition);
			this.RemoveSelf();
			yield break;
		}
