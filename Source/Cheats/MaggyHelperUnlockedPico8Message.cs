...
/// </summary>
public class DZUnlockedPico8Message : Entity
{
    private float alpha;
    private string text;
    ...
    public DZUnlockedPico8Message(Action callback = null)
    {
    ...
    public override void Render()
    {
        ...
        GFX.Gui["pico8"].DrawJustified(Celeste.TargetCenter.Position + new Vector2(0f, -64f * (1f - num) - 16f), new Vector2(0.5f, 1f), Color.White * num);
        Vector2 position = Celeste.TargetCenter.Position + new Vector2(0f, 64f * (1f - num) + 16f);
        Vector2 vector = ActiveFont.Measure(text);
        ActiveFont.Draw(text, position, new Vector2(0.5f, 0f), Vector2.One, Color.White * num);
        if (waitForKeyPress)
        {
            GFX.Gui["textboxbutton"].DrawCentered(Celeste.TargetCenter.Position + new Vector2(vector.X / 2f + 32f, vector.Y + 48f + (float)((timer % 1f < 0.25f) ? 6 : 0)));
        }
    }
}
