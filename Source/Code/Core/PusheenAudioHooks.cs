...
/// <summary>
/// Redirects vanilla audio event paths to their custom Pusheen variants.
...
/// this hook swaps them for the corresponding pusheen/ events.
/// </summary>
public static class PusheenAudioHooks
{
    ...
    public static void Unload()
    {
        ...
        Logger.Log(LogLevel.Info, "MaggyHelper", "PusheenAudioHooks unloaded");
    }
    ...
    /// <summary>
    /// Replaces vanilla audio event paths with their Pusheen variants.
    /// Already-pusheen paths are left untouched to prevent double-redirects.
    /// </summary>
    private static string ReplacePath(string path)
    {
        ...
        // Already a pusheen event — don't double-redirect
        if (path.Contains("/pusheen/"))
            return path;
