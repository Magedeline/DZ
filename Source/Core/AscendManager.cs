...
[CustomEntity(ids: "DZ/AscendManagerBeyond")]
[Tracked(true)]
[HotReloadable]
public class AscendManagerBeyond : Entity
{
    ...
    public AscendManagerBeyond(EntityData data, Vector2 offset)
    ...
    private IEnumerator Routine()
    {
        ...
        if (manager.arrivial && manager.Ch9Ending)
        {
            GiygasStreaks giygasStreaks = new(manager);
            manager.Scene.Add(giygasStreaks);
            VoidBackground voidBg = new(manager);
            manager.Scene.Add(voidBg);
        }
        else
        {
        ...
        player.Sprite.Play("launch");
        Audio.Play("event:/Mods/pusheen/char/kirby/summit_flytonext", player.Position);
        yield return 0.25f;
        ...
        if (manager.introLaunch)
            manager.level.Add(new HeightDisplayMod(-1));
    }
    ...
    {
        private const float MinSpeed = 800f;
        private const float MaxSpeed = 2400f;
        public float Alpha = 1f;
        private readonly Particle[] particles = new Particle[100];
        private readonly List<MTexture> textures;
        private readonly AscendManagerBeyond manager;

        public GiygasStreaks(AscendManagerBeyond manager)
        {
            ...
            {
                float x = (float)(160.0 + (double)Calc.Random.Range(40f, 180f) * Calc.Random.Choose(-1, 1));
                float y = Calc.Random.NextFloat(500f);
                float num = Calc.ClampedMap(Math.Abs(x - 160f), 0.0f, 180f, 0.4f) * Calc.Random.Range(800f, 2400f);
        ...
        public override void Update()
        {
            ...
            for (int index = 0; index < particles.Length; ++index)
            {
                ...
                particles[index].WaveOffset += Engine.DeltaTime * 2f;
            }
        }
        ...
        public override void Render()
        {
            ...
            for (int index = 0; index < textures.Count; ++index)
                alphaColors[index] = Color.White * num * 0.7f;

            for (int index = 0; index < particles.Length; ++index)
            {
                Vector2 position2 = particles[index].Position;
                position2.X = AscendManagerBeyond.Mod(position2.X + MathF.Sin(particles[index].WaveOffset) * 20f, 320f);
                position2.Y = AscendManagerBeyond.Mod(position2.Y, 500f) - 150f;
                Vector2 vector2_1 = position2 + position1;
                Vector2 vector2_2 = new Vector2
                {
                    X = Calc.ClampedMap(particles[index].Speed, 800f, 2400f, 1.2f, 0.3f),
                    Y = Calc.ClampedMap(particles[index].Speed, 800f, 2400f, 1.5f, 3f)
                };
                MTexture texture = textures[particles[index].Index % textures.Count];
                Color alphaColor = alphaColors[particles[index].Index % textures.Count];
                texture.DrawCentered(vector2_1, alphaColor, vector2_2);
            }
        }
    ...
    public class VoidBackground : Entity
    {
        public float Alpha = 1f;
        private readonly AscendManagerBeyond manager;
        private readonly List<MTexture> depthLayers;
        private readonly float[] scrollSpeeds;
        private readonly float[] scrollOffsets;

        public VoidBackground(AscendManagerBeyond manager)
        {
            this.manager = manager;
            Depth = -1000020;
            ...
            {
                scrollSpeeds[i] = 30f + (i * 20f);
                scrollOffsets[i] = Calc.Random.NextFloat(1000f);
            }
        }
        ...
        public override void Update()
        {
            ...
            for (int i = 0; i < scrollOffsets.Length; i++)
                scrollOffsets[i] += scrollSpeeds[i] * Engine.DeltaTime;
        }

        public override void Render()
        {
            float alpha = (manager != null ? manager.fade : 1f) * Alpha;
            Vector2 cameraPos = (Scene as Level).Camera.Position;
            
            for (int i = 0; i < depthLayers.Count; i++)
            {
                MTexture layer = depthLayers[i];
                float layerAlpha = alpha * (0.3f + (i * 0.1f));
                Vector2 offset = new Vector2(0f, AscendManagerBeyond.Mod(scrollOffsets[i], layer.Height));
                
                layer.Draw(cameraPos - offset, Vector2.Zero, Color.White * layerAlpha);
                layer.Draw(cameraPos - offset + new Vector2(0f, layer.Height), Vector2.Zero, Color.White * layerAlpha);
            }
        }
    }
    ...
    public class Streaks : Entity
    {
        ...
        public Streaks(AscendManagerBeyond manager)
        {
            this.manager = manager;
            if (manager == null || !manager.Dark || !manager.arrivial)
            {
                colors = new Color[2]
                {
                    Color.White,
        ...
        public override void Render()
        {
            ...
            {
                Vector2 position2 = particles[index].Position;
                position2.X = AscendManagerBeyond.Mod(position2.X, 320f);
                position2.Y = AscendManagerBeyond.Mod(position2.Y, 436f) - 128f;
                Vector2 vector2_1 = position2 + position1;
                Vector2 vector2_2 = new Vector2
                {
    ...
    public class Clouds : Entity
    {
        ...
        public Clouds(AscendManagerBeyond manager)
        {
            this.manager = manager;
        ...
        public override void Render()
        {
            ...
            {
                Vector2 position2 = particles[index].Position;
                position2.Y = AscendManagerBeyond.Mod(position2.Y, 900f) - 360f;
    ...
    private class Fader : Entity
    {
        ...
        public Fader(AscendManagerBeyond manager)
        {
            this.manager = manager;
            Depth = -1000010;
        }
