...
public static class CelesteDSideHooks
{
    ...
    private static void InstallCrystalHeartHook()
    {
        ...
        if (target == null)
        {
            Logger.Log(LogLevel.Warn, "DZ", "Failed to find HeartGem.Collect method for hook installation.");
            return;
        }
    ...
    private static void InstallDSideILHooks()
    {
        ...
        {
            Logger.Log(LogLevel.Warn, "DZ", $"Failed to install D-Side IL hooks: {ex.Message}");
        }
    }
    ...
    private static void InstallCrystalHeartCollectILHook()
    {
        ...
        if (target == null)
        {
            Logger.Log(LogLevel.Warn, "DZ", "Failed to find HeartGem.Collect method for IL hook installation.");
            return;
        }

        _crystalHeartCollectILHook = new ILHook(target, IL_HeartGem_Collect);
        Logger.Log(LogLevel.Debug, "DZ", "HeartGem.Collect IL hook installed");
    }
    ...
    private static void IL_HeartGem_Collect(ILContext il)
    {
        try
        {
                ...
                instr => instr.MatchLdloc(0)))
            {
                ...
                Logger.Log(LogLevel.Debug, "DZ", "IL_HeartGem_Collect: patch point located");
                patches++;
            }

            if (patches == 0)
            {
                Logger.Log(LogLevel.Debug, "DZ", "IL_HeartGem_Collect: no patches applied (method structure may differ)");
            }
        }
        catch (Exception ex)
        {
            Logger.Log(LogLevel.Warn, "DZ", $"Error in IL_HeartGem_Collect: {ex.Message}");
        }
    }
    ...
    private static void InstallLevelLoadLevelILHook()
    {
        ...
        if (target == null)
        {
            Logger.Log(LogLevel.Warn, "DZ", "Failed to find Level.LoadLevel method for IL hook installation.");
            return;
        }

        _levelLoadLevelILHook = new ILHook(target, IL_Level_LoadLevel);
        Logger.Log(LogLevel.Debug, "DZ", "Level.LoadLevel IL hook installed");
    }
    ...
    private static void IL_Level_LoadLevel(ILContext il)
    {
        try
        {
            ...
            if (patches == 0)
            {
                Logger.Log(LogLevel.Debug, "DZ", "IL_Level_LoadLevel: no target patterns found");
            }
        }
        catch (Exception ex)
        {
            Logger.Log(LogLevel.Warn, "DZ", $"Error in IL_Level_LoadLevel: {ex.Message}");
        }
    }
    ...
    private static void Hook_HeartGem_Collect(orig_HeartGem_Collect orig, HeartGem self, Player player)
    {
        ...
        Logger.Log(LogLevel.Debug, "DZ", $"D-Side crystal heart collected: {heartId}");
    }
    ...
    private static void OnOverworldBegin(On.Celeste.Overworld.orig_Begin orig, Overworld self)
    {
        ...
        {
            Logger.Log(LogLevel.Warn, "DZ", $"Failed to initialize D-Side overworld state: {ex.Message}");
        }
    }
    ...
    private static IEnumerator OnChapterPanelEnter(On.Celeste.OuiChapterPanel.orig_Enter orig, OuiChapterPanel self, Oui from)
    {
        ...
        {
            Logger.Log(LogLevel.Warn, "DZ", $"Failed to initialize D-Side panel visibility: {ex.Message}");
        }
    }
    ...
    private static IEnumerator OnChapterPanelLeave(On.Celeste.OuiChapterPanel.orig_Leave orig, OuiChapterPanel self, Oui next)
    {
        ...
        if (AreaModeExtender.IsOurMap(area))
        {
            ...
            {
                Logger.Log(LogLevel.Warn, "DZ", $"Failed to save D-Side panel state: {ex.Message}");
            }
        }
    ...
    private static void OnLevelLoadLevel(On.Celeste.Level.orig_LoadLevel orig, Level self, Player.IntroTypes introType, bool isFromLoader)
    {
        ...
        {
            Logger.Log(LogLevel.Warn, "DZ", $"Failed to initialize D-Side level state: {ex.Message}");
        }
    }
    ...
    //     catch (Exception ex)
    //     {
    //         Logger.Log(LogLevel.Warn, "DZ", $"Failed to ensure D-Side save structures: {ex.Message}");
    //     }
    // }
    ...
    public static void UnlockDSide(AreaData area, int modeIndex = AreaModeExtender.MODE_DSIDE)
    {
        ...
        Logger.Log(LogLevel.Info, "DZ", $"Unlocked {AreaModeExtender.GetSideLabel(modeIndex)}-Side for {area.SID}");
    }
}
