...
namespace DZ
{
	...
	public class HeartgemDarkness : Entity
	{
		...
		public HeartgemDarkness()
		{
			base.Add(this.sprite = GFX.SpriteBank.Create("HeartgemDarkness"));
			this.sprite.Play("appear", false, false);
			base.Add(this.hands = GFX.SpriteBank.Create("HeartgemHands"));
		...
		public IEnumerator Appear(WindSnowFG windSnowFG = null)
		{
			...
			this.Visible = true;
			base.Scene.Add(this.blackness = new HeartgemDarkness.Blackness());
		...
		public override void Update()
		{
			...
			// If tracking a breathing game, sync anxiety from its panic level
			if (breathingGame != null)
			{
				...
				{
					this.anxiety = breathingGame.PanicLevel;
				}
			}
		...
		private HeartgemDarkness.Blackness blackness;
