namespace DZ
{
	...
	public class HeightDisplayMod : Entity
	{
		...
		public HeightDisplayMod(int index)
		{
			...
			this.index = index;
			string name = "DZ_CH9_HEIGHT_" + ((index < 0) ? "START" : index.ToString());
		...
		private IEnumerator routine()
		{
			...
			{
				Audio.Play("event:/DZ/game/09_beyondsummit/altitude_count");
			}
		...
		private void stepAudioProgression()
		{
			...
			if (!this.setAudioProgression && this.index >= 0 && session.Area.Mode == AreaMode.Normal)
			{
				...
				if (num <= 5)
				{
					session.Audio.Music.Event = "event:/DZ/music/lvl9/main";
					session.Audio.Music.Progress = num;
				}
				else
				{
					session.Audio.Music.Event = "event:/DZ/music/lvl9/final_ascent";
				}
		...
		public override void Render()
		{
			...
			if (this.drawText)
			{
				...
				int currentApproach = (int)this.approach;
				if (currentApproach != _lastCachedApproach)
				{
					_lastCachedApproach = currentApproach;
					_cachedApproachString = currentApproach.ToString();
				}
				ActiveFont.Draw(_cachedApproachString, vector4 + Vector2.UnitX * (this.leftSize + this.numberSize * 0.5f) * scaleFactor, new Vector2(0.5f, 0.5f), scale, color);
			}
		}
		...
		// Cached height string to avoid per-frame ToString allocation
