namespace DZ
{
    ...
    public class WaveFazePage02 : WaveFazePage
    {
        ...
        public override IEnumerator Routine()
        {
            ...
            {
                "WAVEFAZE_PAGE2_LIST_A",
                "WAVEFAZE_PAGE2_LIST_B",
                "WAVEFAZE_PAGE2_LIST_C",
                "WAVEFAZE_PAGE2_LIST_D",
                "WAVEFAZE_PAGE2_LIST_E",
                "WAVEFAZE_PAGE2_LIST_F"
            };

            foreach (string key in listKeys)
            {
                list = FancyText.Parse(Dialog.Get(key), Width, 32, 1f, Color.Black * 0.7f);
                listIndex = 0;
                float delay = 0f;
        ...
        public override void Render()
        {
            ...
            if (impossibleEase <= 0f || Presentation?.Gfx == null)
                return;
            ...
            Matrix transformMatrix = Matrix.CreateRotationZ(-0.5f + Ease.CubeIn(1f - impossibleEase) * 8f) * Matrix.CreateTranslation(Width - 500, Height - 600, 0f);
            ...
            ActiveFont.Draw(Dialog.Clean("WAVEFAZE_PAGE2_STUPID"), Vector2.Zero, new Vector2(0.5f, 0.5f), Vector2.One * (2f + (1f - impossibleEase) * 0.5f), Color.Black * impossibleEase);
            Draw.SpriteBatch.End();
