...
namespace Celeste.Cutscenes
{
	...
	public class CS06_Gondola : CutsceneEntity
	{
		...
		public CS06_Gondola(NPC theo, GondolaMod gondola, Player player) : base(false, true)
		{
			this.theo = theo;
			this.gondola = new GondolaWrapper(gondola);
			this.player = player;
		...
		private IEnumerator Cutscene()
		{
			...
			Audio.SetMusic("event:/Mods/pusheen/music/lvl1/magolor", true, true);
		...
		private IEnumerator PullLever()
		{
			...
			tween.OnUpdate = delegate(Tween t)
			{
				if (Audio.CurrentMusic == "event:/Mods/pusheen/music/lvl1/magolor")
				{
		...
		{
			Audio.SetMusic("event:/Mods/pusheen/music/lvl6/minigame", true, true);
		...
		private IEnumerator DarknessAppears()
		{
			...
			Audio.Play("event:/Mods/pusheen/game/06_stronghold/gondola_scaryhair_01", this.gondola.Position);
		...
		private IEnumerator DarknessConsumes()
		{
			this.Level.Shake(0.3f);
			Input.Rumble(RumbleStrength.Medium, RumbleLength.Medium);
			Audio.Play("event:/Mods/pusheen/game/06_stronghold/gondola_scaryvines_02", this.gondola.Position);
			if (this.gondolaDarkness != null)
			{
		...
		private IEnumerator CantBreath()
		{
			this.Level.Shake(0.3f);
			Input.Rumble(RumbleStrength.Strong, RumbleLength.Medium);
			Audio.Play("event:/Mods/pusheen/game/06_stronghold/gondola_scaryvines_03", this.gondola.Position);
