...
namespace Celeste.Cutscenes
{
    ...
    public class CS07_GenocideVisionIntro : CutsceneEntity
    {
        ...
        private IEnumerator Cutscene(Level level)
        {
            ...
            yield return Textbox.Say("DZ_CH7_GENO_VORTEX_START");
            ...
            while (!knifeCollected || !locketCollected)
            {
                ...
                {
                    yield return CollectProp(level, realKnife, "DZ_CH7_GENO_KNIFE_PICKUP", () => knifeCollected = true);
                }
                else if (!locketCollected && mainChara.ConfirmPressed && Vector2.Distance(mainChara.Position, heartLocket.Position) <= 16f)
                {
                    yield return CollectProp(level, heartLocket, "DZ_CH7_GENO_LOCKET_PICKUP", () =>
                    {
            ...
            yield return 0.15f;
            yield return Textbox.Say("DZ_CH7_GENO_FLOWEY_WRONGNESS");
