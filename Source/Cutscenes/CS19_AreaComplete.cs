        ...
        level.OnEndOfFrame += () =>
        {
            ...
            else
            {
                ...
                if (currentSid.Equals(Chapter19Sid, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    {
                        DZModule.SaveData.VoidMoonUnlocked = true;
                        DZModule.SaveData.PendingUnlockChapter20OnRestart = true;
                        Logger.Log(LogLevel.Info, "DZ", "Void Moon unlocked on overworld mountain (Chapter 19 → 20 transition)");
                    }
