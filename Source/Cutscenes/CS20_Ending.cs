...
namespace Celeste.Cutscenes
{
	...
	public class CS21_Ending : CutsceneEntity
	{
		// Token: 0x06000C7A RID: 3194 RVA: 0x00029E54 File Offset: 0x00028054
		public CS21_Ending(Player player) : base(false, true)
		{
			base.Tag = Tags.HUD;
		...
		private IEnumerator Cutscene(Level level)
		{
			...
			yield return 1f;
			this.Atlas = Atlas.FromAtlas(Path.Combine("Graphics", "Atlases", "FinalCutscenes"), Atlas.AtlasDataFormat.PackerNoAtlas);
			// Filter to only get numbered frames (0-21), excluding UI elements like "21-window", "21-picture", etc.
			var allSubtextures = this.Atlas.GetAtlasSubtextures("");
			this.Frames = allSubtextures
				.Where(t => {
					string name = Path.GetFileNameWithoutExtension(t.AtlasPath);
					// Only include pure numeric names (animation frames)
					return int.TryParse(name, out _);
				})
				.OrderBy(t => int.Parse(Path.GetFileNameWithoutExtension(t.AtlasPath)))
				.ToList();
			...
			yield return 2f;
			this.cinIntro = Audio.Play("event:/desolozantas/final_content/music/lvl21/cinematic/end_intro");
			Audio.SetAmbience(null, true);
			this.counting = true;
			...
			Audio.SetMusic("event:/desolozantas/final_content/music/lvl20/cinematic/end", true, false);
			...
			yield return 1f;
			yield return Textbox.Say("CH21_END_CINEMATIC", new Func<IEnumerator>[]
			{
		...
		private IEnumerator ShowPicture()
		{
			...
			this.center = global::Celeste.Celeste.TargetCenter;
			...
			this.ok.Visible = false;
			yield return 0.1f;
			this.picture.Origin = global::Celeste.Celeste.TargetCenter;
			this.picture.Position = global::Celeste.Celeste.TargetCenter;
			...
			for (float p = 0f; p < 1f; p += Engine.DeltaTime / 0.4f)
			{
				...
				this.picture.Position = global::Celeste.Celeste.TargetCenter + Vector2.UnitY * 120f * (1f - Ease.CubeOut(p));
				this.picture.Color = Color.Black * p;
				yield return null;
			}
			this.picture.Position = global::Celeste.Celeste.TargetCenter;
			this.attachment.Visible = false;
		...
		public override void OnEnd(Level level)
		{
			...
			{
				Audio.SetMusic("event:/desolozantas/final_content/music/lvl20/cinematic/end", true, true);
			}
			Audio.SetMusicParam("end", 1f);
			this.frame = 21;
			this.zoom = 1f;
		...
		private IEnumerator EndingRoutine()
		{
			...
			// Transition to the white cymbal fade / credits cutscene
			Player player = this.Level.Tracker.GetEntity<Player>();
			this.Level.Add(new CS21_Cast(player));
			this.RemoveSelf();
			yield break;
		}
		...
		{
			// Always draw black background first to cover everything
			...
			if (this.Atlas != null && this.Frames != null && this.frame < this.Frames.Count)
			{
				...
				// Calculate scale to ALWAYS fill the screen (cover mode)
				// Account for zoom so even at zoom=0.75 we still cover the screen
				float screenWidth = 1920f;
				float screenHeight = 1080f;
				
				// Base scale to fill screen
				float scaleX = screenWidth / (float)mtexture.Width;
				float scaleY = screenHeight / (float)mtexture.Height;
				float baseScale = Math.Max(scaleX, scaleY);
				
				// Apply zoom but ensure minimum scale fills screen
				// When zoom < 1.0, we need to compensate to still fill the screen
				float minScaleToFill = baseScale;
				float effectiveScale = baseScale * this.zoom;
				if (effectiveScale < minScaleToFill)
				{
					effectiveScale = minScaleToFill;
				}
				
				Vector2 screenCenter = new Vector2(screenWidth / 2f, screenHeight / 2f);
				
				// Draw only the main frame texture (no linked texture to avoid overlapping)
				mtexture.DrawCentered(screenCenter, Color.White, effectiveScale);
		...
		private Vector2 center = global::Celeste.Celeste.TargetCenter;
