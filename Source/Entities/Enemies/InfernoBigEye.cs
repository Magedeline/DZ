...
namespace Celeste
{
    [CustomEntity(ids: "DZ/InfernoBigEye")]
    public class InfernoBigEye : Entity
    {
        ...
        public InfernoBigEye(EntityData data, Vector2 offset)
            : base(data.Position + offset)
        {
            Add(sprite = GFX.SpriteBank.Create("temple_eyeball"));
            Add(pupil = new Image(GFX.Game["danger/DZ/templeeye/pupil"]));
            pupil.CenterOrigin();
        ...
        {
            if (!(h.Entity is MaddyCrystal))
                return;
            MaddyCrystal entity = h.Entity as MaddyCrystal;
            ...
            Audio.Play("event:/DZ/game/07_inferno/eyewall_destroy", Position);
        ...
        {
            InfernoBigEye infernoBigEye = this;
            infernoBigEye.bursting = true;
            Level level = infernoBigEye.Scene as Level;
            level.StartCutscene(infernoBigEye.OnSkip, false, true);
            ...
            infernoBigEye.Add(tween);
            Player player = infernoBigEye.Scene.Tracker.GetEntity<Player>();
            MaddyCrystal entity = infernoBigEye.Scene.Tracker.GetEntity<MaddyCrystal>();
            if (player != null)
            {
            ...
            infernoBigEye.Add(new Coroutine(level.ZoomTo(entity.TopCenter - level.Camera.Position, 2f, 0.5f)));
            infernoBigEye.Add(new Coroutine(entity.Shatter()));
            foreach (TempleEye templeEye in infernoBigEye.Scene.Entities.FindAll<TempleEye>())
                templeEye.Burst();
            infernoBigEye.sprite.Play("burst");
            infernoBigEye.pupil.Visible = false;
            level.Shake(0.4f);
            yield return 2f;
        ...
        private void OnSkip(Level level) => level.CompleteArea(spotlightWipe: false, skipScreenWipe: false, skipCompleteScreen: false);
        ...
        public override void Update()
        {
            ...
            if (entity1 != null)
            {
                ...
                double x2 = X;
                Audio.SetMusicParam("void_distance", Calc.ClampedMap((float)x1, (float)left, (float)x2));
            }
            ...
            double max = X - 32.0;
            Audio.SetMusicParam("void_distance", Calc.ClampedMap((float)x, (float)min, (float)max, 1f, 0.0f));
        }
