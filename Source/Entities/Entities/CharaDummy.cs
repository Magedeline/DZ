...
namespace Celeste.Entities
{
    ...
    /// </summary>
    [CustomEntity(ids: "DZ/CharaDummy")]
    [HotReloadable]
    public class CharaDummy : Entity
    {
        ...
        public bool AutoAnimateEnabled { get; set; } = true;
        ...
        public CharaDummy(Vector2 position, string dialogKey = null) : base(position)
        {
        ...
        private void InitializeSprite()
        {
            ...
            {
                // Fallback to badeline if chara sprite bank fails
                try
                {
                ...
                {
                    Logger.Log(LogLevel.Error, "CharaDummy", "Failed to create sprite from sprite bank");
                    return;
                }
            }
        ...
        private void InitializeHair()
        {
            try
            {
                MTexture hairTexture = GFX.Game["characters/DZ/chara/hair00"];
        ...
        private void InitializeTalkComponent()
        {
            ...
            {
                Logger.Log(LogLevel.Warn, "CharaDummy", $"Failed to initialize talk component: {ex.Message}");
            }
        }
        ...
        {
            if (Sprite == null || !AutoAnimateEnabled) return;
        ...
        {
            // Estimate speed from position changes
            return (Position - (Position - new Vector2(FloatSpeed * Engine.DeltaTime, 0)));
        }
