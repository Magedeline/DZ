...
namespace Celeste.Entities
{
    ...
    /// </summary>
    [CustomEntity(ids: "DZ/CowboyTargetPractice")]
    [Tracked]
    [HotReloadable]
    public class CowboyTargetPractice : Entity
    {
        ...
        private IEnumerator evaluatePerformance()
        {
            ...
            if (accuracy >= 90f && targetsHit >= requiredTargets)
            {
                ...
                setSessionFlag("DZ_CH11_COWBOY_GUN_PRACTICES_TRUELY_PASSED");
                Audio.Play("event:/ui/game/perfect_complete");
            }
            else if (accuracy >= 70f && targetsHit >= requiredTargets)
            {
                resultRank = "passed";
                setSessionFlag("DZ_CH11_COWBOY_GUN_PRACTICES_PASSED");
                Audio.Play("event:/ui/game/complete");
            }
            else if (accuracy >= 50f)
            {
                resultRank = "B";
                setSessionFlag($"DZ_CH11_COWBOY_GUN_PRACTICES_TRY_AGAIN_B");
            }
            else if (accuracy >= 30f)
            {
                resultRank = "C";
                setSessionFlag($"DZ_CH11_COWBOY_GUN_PRACTICES_TRY_AGAIN_C");
            }
            else
            {
                resultRank = "A";
                setSessionFlag($"DZ_CH11_COWBOY_GUN_PRACTICES_TRY_AGAIN_A");
            }
        ...
        private void showResultDialog()
        {
            ...
            string dialogKey = $"DZ_CH11_COWBOY_GUN_PRACTICES_{resultRank.ToUpper()}";
