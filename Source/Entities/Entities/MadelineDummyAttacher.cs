...
{
    /// <summary>
    /// Attaches a floating Madeline dummy to the player as a companion.
    /// The dummy hovers around the player and follows their movement.
    /// Intended for later DLC use.
    /// </summary>
    [CustomEntity(ids: "DZ/MadelineDummyAttacher")]
    [HotReloadable]
    public class MadelineDummyAttacher : Component
    {
        private MadelineDummy madelineDummy;
        ...
        public MadelineDummyAttacher() : base(true, true)
        {
        }

        public MadelineDummyAttacher(EntityData data, Vector2 offset) : base(true, true)
        {
        ...
        public override void Added(Entity entity)
        {
            ...
            {
                // Create the Madeline dummy
                madelineDummy = new MadelineDummy(player.Position);
                level.Add(madelineDummy);
            }
        }
        ...
        public override void Update()
        {
            ...
            if (madelineDummy == null || player == null || player.Scene == null)
                return;
            ...
            madelineDummy.Position = Calc.Approach(madelineDummy.Position, targetPosition, 120f * Engine.DeltaTime);
            
            // Make dummy face the same direction as player
            if (madelineDummy.Sprite != null)
            {
                madelineDummy.Sprite.Scale.X = player.Facing == Facings.Right ? -1f : 1f;
            }
        }
        ...
        public override void Removed(Entity entity)
        {
            ...
            if (madelineDummy != null && madelineDummy.Scene != null)
            {
                madelineDummy.RemoveSelf();
            }
        }
