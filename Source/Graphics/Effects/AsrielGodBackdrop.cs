...
namespace Celeste.Effects
{
    ...
    /// </summary>
    [CustomBackdrop("DZ/AsrielGodBackdrop")]
    [HotReloadable]
...
#pragma warning restore CS0414
        ...
        public AsrielGodBackdrop()
        {
            ...
            try
            {
                ...
                if (gridTexture == null)
                {
                    Logger.Log(LogLevel.Warn, "DZ/AsrielGodBackdrop", "Grid texture 'bgs/20/asriel/bg00' not found");
                }
            }
            catch (Exception ex)
            {
                Logger.Log(LogLevel.Error, "DZ/AsrielGodBackdrop", $"Failed to load grid texture: {ex.Message}");
                gridTexture = null;
            }
