...
namespace DZ
{
    [CustomEntity(ids: "DZ/StrawberryExt")]
    [Tracked]
    public class StrawberryExt : Entity
  {
    ...
    public StrawberryExt(EntityData data, Vector2 offset, EntityID gid)
    {
      this.ID = gid;
      ...
      this.Pink = data.Name == "memorialTextController" || data.Name == "goldenBerry";
      this.Moon = data.Bool("moon");
      this.PopStar = data.Bool("popstar");
    ...
    public override void Added(Scene scene)
    {
      ...
      {
        this.sprite = GFX.SpriteBank.Create(this.GetSpriteName(true));
        this.sprite.Color = Color.White * 0.8f;
      }
      else
        this.sprite = GFX.SpriteBank.Create(this.GetSpriteName(false));
      ...
      this.Add((Component) (this.bloom = new BloomPoint(this.Golden || this.Pink || this.Moon || this.PopStar || this.isGhostBerry ? 0.5f : 1f, 12f)));
    ...
    private string GetSpriteName(bool ghost)
    {
      if (this.PopStar)
        return ghost ? "popstarghostberry" : "popstarberry";
      if (this.Moon)
        return ghost ? "moonghostberry" : "moonberry";
      if (this.Pink)
        return ghost ? "ghostpinkplatinumberry" : "pinkplatinumberry";
      if (this.Golden)
        return ghost ? "goldghostberry" : "goldberry";
      return ghost ? "ghostberry" : "strawberry";
    }
