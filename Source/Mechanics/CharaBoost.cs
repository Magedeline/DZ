...
[CustomEntity(ids: "DZ/CharaBoost")]
[Tracked]
...
public class CharaBoost : Entity
{
    ...
    private IEnumerator BoostRoutine(Player player)
    {
        ...
        if (finalCh19Boost)
        {
            Audio.Play("event:/Mods/pusheen/new_content/char/chara/booster_finalfinal_part1", Position);
        }
        else if (!finalBoost)
        {
        ...
        else
        {
            if (finalCh19Boost)
            {
                Ch9FinalBoostSfx = Audio.Play("event:/Mods/pusheen/new_content/char/chara/booster_finalfinal_part2", Position);
            }
