...
namespace Celeste.UI
{
    ...
    public class DebugRoomWarpMenu : Entity
    {
        ...
        private void ScanRooms()
        {
            ...
            roomNames.Sort();
            Logger.Log(LogLevel.Info, "DZ/Debug", $"DebugRoomWarpMenu: scanned {roomNames.Count} rooms");
        }
        ...
        private void Open()
        {
            ...
            Logger.Log(LogLevel.Info, "DZ/Debug", "DebugRoomWarpMenu opened");
        }
        ...
        private void Close()
        {
            ...
            Logger.Log(LogLevel.Info, "DZ/Debug", "DebugRoomWarpMenu closed");
        }
        ...
        private void WarpToRoom(string roomName)
        {
            ...
            try
            {
                Logger.Log(LogLevel.Info, "DZ/Debug", $"Warping to room: {roomName}");

                // Find the room's spawn point
                var targetLevelData = level.Session.MapData.Get(roomName);
                if (targetLevelData == null)
                {
                    Logger.Log(LogLevel.Warn, "DZ/Debug", $"Room '{roomName}' not found in map data");
                    return;
                }
                ...
                Logger.Log(LogLevel.Info, "DZ/Debug", $"Warp to '{roomName}' scheduled");
            }
            catch (Exception ex)
            {
                Logger.Log(LogLevel.Error, "DZ/Debug", $"Failed to warp to '{roomName}': {ex.Message}");
            }
        }
